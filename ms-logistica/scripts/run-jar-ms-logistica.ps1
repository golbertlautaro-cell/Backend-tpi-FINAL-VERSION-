# Build and run ms-logistica as a background Java process (independent of Maven interactive run)
param(
    [string]$mvnArgs = "-DskipTests package",
    [string]$javaOpts = "-Duser.timezone=UTC -Xms256m -Xmx512m"
)

Write-Host "Building ms-logistica with: mvn $mvnArgs"
...
if ($null -eq $jar) {
 ...
 $logFileOut = Join-Path -Path .\logs -ChildPath "ms-logistica.out.log"
 $logFileErr = Join-Path -Path .\logs -ChildPath "ms-logistica.err.log"
...
Write-Host "Started ms-logistica (check logs with Get-Content -Tail 50 .\logs\ms-logistica.out.log -Wait)"