# Build and run ms-solicitudes as a background Java process (independent of Maven interactive run)
param(
    [string]$mvnArgs = "-DskipTests package",
    [string]$javaOpts = "-Duser.timezone=UTC -Xms256m -Xmx512m"
)

Write-Host "Building ms-solicitudes with: mvn $mvnArgs"
...
if ($null -eq $jar) {
 ...
 $logFileOut = Join-Path -Path .\logs -ChildPath "ms-solicitudes.out.log"
 $logFileErr = Join-Path -Path .\logs -ChildPath "ms-solicitudes.err.log"
...
Write-Host "Started ms-solicitudes (check logs with Get-Content -Tail 50 .\logs\ms-solicitudes.out.log -Wait)"