...
public class ClienteController {
    ...
    @Operation(
        summary = "Listar contenedores de cliente",
        description = "Retorna la lista de contenedores disponibles para un cliente"
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Lista de contenedores obtenida"),
        @ApiResponse(responseCode = "404", description = "Cliente no encontrado"),
        @ApiResponse(responseCode = "401", description = "No autorizado")
    })
    @GetMapping("/{idCliente}/contenedores")
    public List<ContenedorResponse> listarContenedores(
        ...
        return List.of(
            new ContenedorResponse(1L, "Refrigerado", 15000.0, 30.0),
            new ContenedorResponse(2L, "Seco", 20000.0, 35.0)
        );
    }
}
